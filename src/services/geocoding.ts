...
export const geocodeLocation = async (city: string, dept: string): Promise<[number, number] | null> => {
  // 1. Try Local/Sync resolution first
  const localResult = resolveLocationLocal(city, dept);
  if (localResult) return localResult;

  // 2. Last Resort: Nominatim API
  let query = "";
...
// Purely synchronous location resolution (Local DB + Heuristics)
export const resolveLocationLocal = (city: string, dept: string): [number, number] | null => {
    ...
    // 2. Fallback: Check if Department Code exists and return Dept Center
    if (dept && DEPARTMENT_DATA[dept.padStart(2, '0')]) {
        ...
        // If City is NOT empty but not found locally (e.g. "Paris 12e", "Lieu-dit X")
        // User wants speed -> Fallback to Dept Center.
        return deptInfo.center;
    }
